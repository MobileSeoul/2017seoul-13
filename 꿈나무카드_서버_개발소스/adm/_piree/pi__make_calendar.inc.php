...
*/
	...
	//=======================================================
	// 기본__설정_정보_파일__경로
	$piree_base_config_path = PIREE_LIB_PATH ."/date/";
	...
	IF ( file_exists($date_data_file))
	{
		...
		IF ( count($dates) > 0 )
		{
			...
			WHILE ( list($key, $val) = each($dates))
			{
				...
				//=================================================
				// ROW__유무_확인하는__쿼리문
				$sql	 = "SELECT COUNT(*) AS exist FROM `". $piree['calendar'] ."` WHERE `date_n` = '". $date_n ."'";
				$row	 = sql_fetch($sql);
				$exist = @$row["exist"];
				...
				IF ( !$exist || $exist < 1)
				{
					...
					//===============================================
					// 입력_쿼리문
					$sql	= "INSERT INTO `". $piree['calendar'] ."` (num,date_n,date_Y,date_m,date_d,date_Ymd,yoil_n,yoil_seq,lunar_Ymd,kor_gapja_s,cha_gapja_s,is_holiday,is_24_jeolki,regi_time_n) ";
