...
?>
<!-- 게시판 목록 시작 { -->
<div class="ctin" style="margin: 10px 10px 0 10px;">
<div id="gbasic">
...
<? $j=count($list);?>
	<div class="bbs-list">
		<ul>
			<? for ($i=0; $i<$j; $i++) {
			?>
			<li<? if ($wr_id == $list[$i]['wr_id']){?> class="ing"<?}?>>
					<span class="w45 num"><? if ($list[$i]['is_notice']){?><strong>공지</strong><?}else{?><?=$list[$i]['num']?><?}?></span>					 
					<p class="bls bsze">
					<?=$list[$i]['icon_reply']?>
					<? if ($is_checkbox) { ?><input type="checkbox" name="chk_wr_id[]" value="<?php echo $list[$i]['wr_id'] ?>" id="chk_wr_id_<?php echo $i ?>" class="ychk"><?}?>
					...
					<? if($list[$i]['icon_new']){?><span class="new">NEW</span><?}?>
					...
					<p>
						<span class="w100 <? if($board['bo_use_sideview']){?>snon<?}?>"><span class="glyphicon glyphicon-user"></span> <? if($board['bo_use_sideview']){?><span class="sname"><?=$list[$i]['name']?></span><?}?><span class="pname"><?=$list[$i]['wr_name']?></span></span>
...
<!-- 페이지 -->
<?php if ($list_href || $is_checkbox || $write_href || $write_href) { ?>
